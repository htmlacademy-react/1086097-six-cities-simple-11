...
export default function RatingInputRadio({rating, index, title, handleInputClick}:RatingInputRadioProps): JSX.Element {
  return (
    <>
      <input className="form__rating-input visually-hidden" name="rating" defaultValue={index} id={`${index}-stars`} type="radio" onClick={handleInputClick} checked={rating === index}/>
